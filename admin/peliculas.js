...
const editarPelicula = (id) => {
  if (
    ...
    axios
      .put("http://localhost:3001/peliculas/" + id, {
        Nombre: nombre.value,
  ...
  axios
    ...
    .then((res) => {
      ...
      contenedorBotones.innerHTML = `
        <button class ='btn btn-warning w-100 mt-3' type='button' onclick='editarPelicula(${id})'>Aceptar</button>`;
...
const borrarPelicula = (id) => {
  if (confirm(`Está seguro que quiere borrar la pelicula?`)) {
    axios
      .delete("http://localhost:3001/peliculas/" + id)
...
const agregarPelicula = (e) => {
  if (
    ...
    e.preventDefault();
    axios
      .post("http://localhost:3001/peliculas/", {
        Nombre: nombre.value,
        Descripcion: descripcion.value,
        Precio: precio.value,
        Actores: actores.value,
